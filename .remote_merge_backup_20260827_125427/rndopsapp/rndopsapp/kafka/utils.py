...
def resolve_budget_head_name(account_head: Optional[str]) -> Optional[str]:
    """
    Resolve any representation of a Budget Head — its docname, its numeric
    `id` (see get_budget_head_id), or its raw `budget_head` label — to the
    canonical docname, so the same head can be matched/compared regardless of
    which form a given row happens to store.

    Args:
        account_head: Budget Head docname, numeric id, or budget_head label.

    Returns:
        str or None: The Budget Head docname if resolved.
    """
    if not account_head:
        return None

    account_head = str(account_head).strip()

    try:
        # 1. Already the docname
        if frappe.db.exists("Budget Head", account_head):
            return account_head

        # 2. Numeric ledger id
        if account_head.isdigit():
            name = frappe.db.get_value("Budget Head", {"id": int(account_head)}, "name")
            if name:
                return name

        # 3. Raw label, with plural/singular fallback
        name = frappe.db.get_value("Budget Head", {"budget_head": account_head}, "name")
        if name:
            return name

        if account_head.endswith("s"):
            name = frappe.db.get_value("Budget Head", {"budget_head": account_head[:-1]}, "name")
        else:
            name = frappe.db.get_value("Budget Head", {"budget_head": account_head + "s"}, "name")
