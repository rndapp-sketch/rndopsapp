...
**Status:** Implemented locally — production handoff notes included  
...
This document records the implemented ICSS backend architecture and the production handoff notes needed to deploy the same behavior safely.
...
- Director approval PDF gate in `Pending Dean Approval`
- final signed PO delivery flow to `PO Delivered`
- MinIO folder validation for ICSS uploaded files
- dynamic put-back engine
- ICSS PO AMC table support
- Kafka module override for ICSS PO re-commit
- defensive backend save fixes for production stability

Production note:

- If production still creates only an ICSS number without saving data, production is likely missing the composite save API/dispatch logic documented in section 23.
- If production throws `NoneType object is not iterable`, production is likely missing the table normalization fixes documented in section 23.
- If production throws missing optional-field errors such as `icss_packing_charges`, production is likely missing the safe `self.get(...)` calculation fixes documented in section 23.
...
- Director approval PDF gate inside `Pending Dean Approval`
- dynamic put-back actions for approval stages
- final PO delivery to `PO Delivered`
...
- Director escalation is active as a Dean-stage PDF gate, not as a separate workflow route.
...
## 23. Production Save Stability Fixes

This section records the backend corrections that made the local ICSS save flow work. Production should include these fixes before frontend handoff/testing.

### 23.1 Blank Draft / Only Application Number Created

Observed production symptom:

```text
Save Draft creates an ICSS number, but parent/child data is not saved.
Example: /app/indent-cum-sanction-sheet/2026060110000873 opens with mostly blank data.
```

Most likely cause:

- Frontend is sending the newer composite payload shape:

```json
{
  "parent": {
    "icss_indent_type": "Annual Maintenance Contract",
    "icss_applicant_webmail_id": "user@iitg.ac.in"
  },
  "child": {
    "doctype": "AMC"
...
- Production backend may still be using only the older flat `save_icss_data(data)` path.
- Older flat save logic sees top-level keys like `parent` and `child`, but those are not DocField names on `Indent Cum Sanction Sheet`.
- If the old path inserts with `ignore_mandatory=True`, Frappe can create only the autoname/application number while ignoring the actual nested data.

Required production correction:

- Production must include `save_icss_composite_data(data)`.
- Frontend should call:

```text
rndopsapp.rndopsapp.doctype.indent_cum_sanction_sheet.indent_cum_sanction_sheet.save_icss_composite_data
```

- For backward compatibility, if `save_icss_data(data)` receives a payload containing `parent` or `child`, backend should redirect to the composite save path instead of silently inserting a blank parent.
...
if isinstance(payload, dict) and ("parent" in payload or "child" in payload):
    return save_icss_composite_data(payload)
```

Expected behavior after fix:

- Parent ICSS fields are saved from `payload["parent"]`.
- Child doctype is resolved from `icss_indent_type`.
- Child document is saved from `payload["child"]`.
- Child is linked using `indent_cum_sanction_sheet_id = parent.name`.
- Parent `sub_doctype_reference` is updated with the child docname.

### 23.2 `NoneType` Object Is Not Iterable

Observed production symptom:

```text
Failed to save Indent Cum Sanction Sheet: 'NoneType' object is not iterable
```

Cause:

- Some table fields can arrive as `None`/`null` from frontend or from old schema/runtime state.
- Python loops such as `for row in self.get("icss_items", [])` still fail if the key exists with value `None`.

Required production correction:

- Every backend table loop must treat `None` as an empty list.
...
For generic DocType table save logic, use:
...
Files/areas that need this defensive pattern:
...
Known table fields affected:

- `icss_items`
- `table_qanf`
- `details_of_items_to_be_purchased`
- `amc_po_table`
- rate contract item rows
- any child table handled by the generic payload saver

Frontend can still send empty tables as `[]`, but backend must not crash if a table is missing or sent as `null`.

### 23.3 Missing Optional Parent Fields

Observed production symptom:

```text
Failed to save Indent Cum Sanction Sheet: 'IndentCumSanctionSheet' object has no attribute 'icss_packing_charges'
```

Cause:

- Current ICSS schema may not contain some old parent calculation fields.
- Direct attribute access like `self.icss_packing_charges` raises an exception when the field does not exist in the live DocType.

Required production correction:

- Optional or legacy fields must be read with `self.get(...)`.
...
Also use `self.get(...)` for optional repair/AMC calculation fields:
...
### 23.4 Parent Must Exist Before Child Link Validation

Observed production symptom:

```text
Could not find Indent Cum Sanction Sheet ID: 2026052210000855
Failed to save sub-doctype record: Could not find Indent Cum Sanction Sheet ID: 2026052210000855
```

Cause:

- `before_save()` was creating/saving the child doctype before the new parent ICSS row existed in the database.
- Child link validation then failed because `indent_cum_sanction_sheet_id` pointed to a parent name that had not been inserted yet.

Required production correction:

- Do not sync/create child doctype from `before_save()` for a new parent document.
...
- New parent ICSS is inserted first.
- Composite save then creates/updates the child after parent exists.
- Child Link field validation succeeds.

### 23.5 Production Deployment Checklist For Save Fixes

These fixes are Python/controller changes. They do not need manual SQL by themselves.

Run after deploying code:

```bash
python3 -m py_compile rndopsapp/rndopsapp/doctype/indent_cum_sanction_sheet/indent_cum_sanction_sheet.py
python3 -m py_compile rndopsapp/rndopsapp/doctype/proprietary_purchase/proprietary_purchase.py
python3 -m py_compile rndopsapp/rndopsapp/doctype/standerdized_purchase/standerdized_purchase.py
python3 -m py_compile rndopsapp/rndopsapp/doctype/repair_replacement/repair_replacement.py
python3 -m py_compile rndopsapp/rndopsapp/doctype/rate_contract/rate_contract.py
bench --site <site-name> migrate
bench --site <site-name> clear-cache
bench restart
```
...
- Save Draft with composite payload creates ICSS parent with actual field values.
- Linked child doctype is created and linked through `indent_cum_sanction_sheet_id`.
- Parent `sub_doctype_reference` is populated.
- Empty/missing child tables do not throw `NoneType object is not iterable`.
- Missing legacy optional fields do not throw attribute errors.
- Reopening the ICSS through `get_icss_fields(doc_name=...)` returns parent data plus nested `child_document`.
