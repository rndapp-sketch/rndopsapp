...
def _ensure_list(data, key):
	"""
	If data[key] is a JSON string, parse it into a list.
	If it's already a list, leave it. Otherwise set to empty list.
	Prevents 'str' object does not support item assignment errors.
	"""
	val = data.get(key)
...
def _assemble_flat_child_rows(data):
	...
	"""
	# -- Pre-parse any JSON string values for child tables --
	# The frontend sometimes sends these as JSON.stringify(rows) which arrive as strings.
	# We must parse them BEFORE trying to index/update them as lists.
	for child_key in ("bank_details_u_r", "qualifications_u_r", "experiences_u_r",
					   "uploaded_documents_u_r", "address_details",
					   "org_address_details_u_r"):
		val = data.get(child_key)
		if isinstance(val, str):
			try:
				parsed = json.loads(val)
				if isinstance(parsed, list):
					data[child_key] = parsed
				# If it parsed to something else, leave it for downstream handling
			except (json.JSONDecodeError, TypeError, ValueError):
				pass  # Leave as-is; downstream code handles str → list conversion
	...
	if bank_row_raw:
		...
		_ensure_list(data, "bank_details_u_r")
		if not data["bank_details_u_r"]:
			data["bank_details_u_r"].append(bank_row)
		elif isinstance(data["bank_details_u_r"][0], dict):
			data["bank_details_u_r"][0].update(bank_row)
		else:
			data["bank_details_u_r"] = [bank_row]
	...
	if edu_row_raw:
		...
		_ensure_list(data, "qualifications_u_r")
		if not data["qualifications_u_r"]:
			data["qualifications_u_r"].append(edu_row)
		elif isinstance(data["qualifications_u_r"][0], dict):
			data["qualifications_u_r"][0].update(edu_row)
		else:
			data["qualifications_u_r"] = [edu_row]
	...
	if exp_row_raw:
		...
		_ensure_list(data, "experiences_u_r")
		if not data["experiences_u_r"]:
			data["experiences_u_r"].append(exp_row)
		elif isinstance(data["experiences_u_r"][0], dict):
			data["experiences_u_r"][0].update(exp_row)
		else:
			data["experiences_u_r"] = [exp_row]
...
def get_existing_registration(email):
	...
	try:
		...
		# 1. First find the Universal User ID by email
		...
		# 2. Try lookup by Universal User reference
		if universal_user:
			existing = frappe.db.get_value(
				"Universal Registration__",
				{"universal_user_u_r": universal_user},
				"name"
			)
			
		# 3. Fallback to direct email search
		if not existing:
			existing = frappe.db.get_value(
				"Universal Registration__",
...
def get_or_load_user_registration():
	...
	try:
		...
		# Step 1: Find the Universal User ID associated with the current session user
		universal_user = None
		try:
			# The session user is typically the email address
			universal_user = frappe.db.get_value(
				"Universal User__",
			...
			if not universal_user:
				# Also check against linked_frappe_user
				universal_user = frappe.db.get_value(
					"Universal User__",
					{"linked_frappe_user": current_user},
					"name"
				)
		except Exception as e:
			print(f"[DEBUG] ✗ Failed to look up Universal User: {e}")
		
		# Strategy 1: Search for existing registration by universal_user_u_r field
		if universal_user:
			try:
				existing = frappe.db.get_list(
					"Universal Registration__",
					filters={"universal_user_u_r": universal_user},
					fields=["name"],
					limit=1
				)
		...
		# Strategy 2: Try by user's email directly on the Universal Registration profile
		if not docname:
			try:
...
def save_universal_registration_data(data=None, **kwargs):
	...
	try:
		...
		address_field_mapping = {
			...
			'address_line_2': 'address_line_2_u_r',
			'country': 'country',
			'is_same_as_permanent': 'is_same_as_permanent_u_r',
		}
		
		current_address_field_mapping = {
			'current_address_type': 'address_type_u_r',
			'current_house_no': 'address_line_1_u_r',
			'current_street_area': 'address_line_2_u_r',
			'current_landmark': 'landmark_u_r',
			'current_pin_code': 'pincode_u_r',
			'current_postal_code': 'pincode_u_r',
			'current_city': 'city_u_r',
			'current_state': 'state_u_r',
			'current_district': 'district_u_r',
			'current_address_line_1': 'address_line_1_u_r',
			'current_address_line_2': 'address_line_2_u_r',
			'current_country': 'country',
		}
		
		# Extract and map permanent address fields
		...
		# Ensure is_same_as_permanent_u_r is strictly 1 or 0 for Frappe Data Storage
		if 'is_same_as_permanent_u_r' in address_fields_in_data:
			is_same_val = address_fields_in_data['is_same_as_permanent_u_r']
			address_fields_in_data['is_same_as_permanent_u_r'] = 1 if str(is_same_val).lower() in ['1', 'true'] else 0
			
		# Extract and map current address fields
		current_address_fields_in_data = {}
		for frontend_field, child_table_value in list(data.items()):
			if frontend_field in current_address_field_mapping:
				doctype_field = current_address_field_mapping[frontend_field]
				current_address_fields_in_data[doctype_field] = child_table_value
				
		# Remove mapped flat fields from parent data
		data = {k: v for k, v in data.items() if k not in address_field_mapping and k not in current_address_field_mapping}
		
		if address_fields_in_data or current_address_fields_in_data:
			if 'address_details' not in data or not isinstance(data['address_details'], list):
				data['address_details'] = []
				
			# Process permanent address (row 0)
			...
			is_same = address_fields_in_data.get('is_same_as_permanent_u_r', 0) == 1
			
			# Process current address (row 1)
			if current_address_fields_in_data and not is_same:
				while len(data['address_details']) < 2:
					data['address_details'].append({})
				data['address_details'][1].update(current_address_fields_in_data)
				
			# Cleanup any current address mistakenly submitted or old from DB if "same as permanent" ticked
			if is_same and len(data['address_details']) > 1:
				data['address_details'] = [data['address_details'][0]]
		...
		for fieldname, value in data.items():
			...
			# Handle Child Tables
			# The frontend may send child-table data as a JSON string
			# (e.g. qualifications = JSON.stringify(rows)). Parse it first.
			if df.fieldtype == "Table" and isinstance(value, str):
				try:
...
def update_universal_registration_data(docname, data):
	...
	try:
		...
		# Pre-parse any JSON-stringified child tables before assembling flat fields
		for child_key in ("bank_details_u_r", "qualifications_u_r", "experiences_u_r",
						   "uploaded_documents_u_r", "address_details",
						   "org_address_details_u_r"):
			val = data.get(child_key)
