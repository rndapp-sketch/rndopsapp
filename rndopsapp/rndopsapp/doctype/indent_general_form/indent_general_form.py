...
def get_indent_general_form_fields(doc_name=None):
    # 1. Fetch Metadata
    meta = frappe.get_meta("Indent General Form")
    fields = []
    ...
    if doc_name:
        doc = frappe.get_doc("Indent General Form", doc_name)
    ...
    client_scripts = []
    try:
        scripts = frappe.get_all("Client Script", filters={"dt": "Indent General Form", "enabled": 1}, fields=["name", "script", "view"])
...
def save_indent_general_form_data(data, files=None, file=None):
    ...
    try:
        ...
        if docname:
            ...
            doc = frappe.get_doc("Indent General Form", docname)
        else:
            print(f"[IGF] CREATE new doc")
            doc = frappe.new_doc("Indent General Form")
        ...
        if attach_field_updates:
            ...
            reload_doc = frappe.get_doc("Indent General Form", doc.name)
...
def get_indent_general_form_workflow_actions(docname):
    wf_name = frappe.db.get_value("Workflow", {"document_type": "Indent General Form", "is_active": 1}, "name")
    if not wf_name:
        return {"actions": []}
