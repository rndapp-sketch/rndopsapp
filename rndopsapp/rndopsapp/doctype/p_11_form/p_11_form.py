...
class P_11Form(Document):
	def validate(self):
		pass

# =============================================================================
# API ENDPOINTS for P_11 Form
# =============================================================================

@frappe.whitelist()
def get_p_11_form_fields(doc_name=None):
	"""
	API to return P_11 Form field metadata, prefill data, and client scripts.
	"""
	meta = frappe.get_meta("P_11 Form")
	fields = []
	...
	if doc_name:
		try:
			doc = frappe.get_doc("P_11 Form", doc_name)
	...
	client_scripts = []
	try:
		scripts = frappe.get_all("Client Script", filters={"dt": "P_11 Form", "enabled": 1}, fields=["name", "script", "view"])
...
@frappe.whitelist()
def save_p_11_form_data(data):
	"""
	Creates or updates a P_11 Form document.
	"""
	...
	try:
		...
		if doc_name and frappe.db.exists("P_11 Form", doc_name):
			doc = frappe.get_doc("P_11 Form", doc_name)
			if doc.docstatus != 0:
				frappe.throw(_("Cannot edit a submitted or cancelled document."))
		else:
			doc = frappe.new_doc("P_11 Form")
			is_new = True
		...
		frappe.db.rollback()
		frappe.log_error(frappe.get_traceback(), "P_11 Form Save Error")
		return {"status": "error", "message": str(e)}

@frappe.whitelist()
def get_p_11_form_workflow_actions(docname):
	doc = frappe.get_doc("P_11 Form", docname)
	current_state = doc.workflow_state or "Draft"
	user_roles = frappe.get_roles(frappe.session.user)

	workflow_name = frappe.db.get_value("Workflow", {"document_type": "P_11 Form", "is_active": 1}, "name")
...
@frappe.whitelist()
def perform_p_11_form_action(docname, action):
	try:
		doc = frappe.get_doc("P_11 Form", docname)
		current_state = doc.workflow_state or "Draft"
		user_roles = frappe.get_roles(frappe.session.user)
		...
		workflow_name = frappe.db.get_value("Workflow", {"document_type": "P_11 Form", "is_active": 1}, "name")
		if not workflow_name:
			frappe.throw("No active workflow found for P_11 Form.")
		...
		# Trigger direct purchase status update if P_11 Form reaches final approved state
		if next_state == "P_11 Approved" and doc.amended_from:
			try:
				frappe.db.set_value("Direct Purchase", doc.amended_from, "workflow_state", "P_11 Approved")
				frappe.db.commit()
			except Exception:
				pass
		...
		return {
			...
			"next_actions": get_p_11_form_workflow_actions(docname)
		}

	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(frappe.get_traceback(), "P_11 Form Action Error")
		return {"status": "error", "message": str(e)}
