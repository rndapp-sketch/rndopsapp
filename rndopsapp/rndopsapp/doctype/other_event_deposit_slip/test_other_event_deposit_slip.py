...
class TestOtherEventDepositSlip(FrappeTestCase):
	def _make_doc(self):
		doc = frappe.new_doc("Other Event Deposit Slip")
		doc.event_title = "ORIGINAL EVENT"
		doc.bank = "ORIGINAL BANK"
		doc.flags.skip_kafka_sync = True
		doc.insert(ignore_permissions=True)
		self.addCleanup(
			frappe.delete_doc, "Other Event Deposit Slip", doc.name,
	...
	def test_non_staff_user_is_blocked(self):
		...
		try:
			self.assertRaises(
				...
				"Other Event Deposit Slip", doc.name, {"bank": "SHOULD NOT APPLY"},
			)
		finally:
	...
	def test_field_update_and_protected_fields_dropped(self):
		...
		result = update_locked_deposit_slip(
			"Other Event Deposit Slip", doc.name,
			changes={
				"bank": "UPDATED BANK",
		...
		self.assertEqual(frappe.db.get_value("Other Event Deposit Slip", doc.name, "bank"), "UPDATED BANK")
