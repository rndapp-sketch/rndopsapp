...
def get_initial_endorsement_html(docname, print_format):
    """
    Generates the initial HTML content of the endorsement using a Frappe Print Format.
    This content will be shown in the client-side HTML Editor for review.
    """
    try:
        doc = frappe.get_doc('Project Proposal', docname)

        # Validate that the print format exists
        if not frappe.db.exists("Print Format", print_format):
            frappe.throw(f"Print Format '{print_format}' does not exist.")

        # Generate HTML using the specified print format
        # The `doctype` and `name` are passed to the Jinja template of the print format
        endorsement_html = frappe.get_print(
            'Project Proposal',
            doc.name,
            print_format=print_format,
            as_html=True,
            no_letterhead=False # Use letterhead if desired for the preview
        )

        return {"html": endorsement_html}
    except Exception as e:
...
def save_edited_endorsement_and_submit_for_approval(docname, edited_content, print_format):
    ...
    generates PDF, and forwards it to the 'Dean, RnD' role. No email.
    """
    try:
        doc = frappe.get_doc('Project Proposal', docname)

        # 1. Update the 'edited_endorsement_content' field
        # Make sure you have an 'edited_endorsement_content' HTML/Long Text field in your DocType
        doc.edited_endorsement_content = edited_content

        # 2. Update endorsement_status and save
        ...
        pdf_file = None
        try:
            # Generate PDF based on the *saved* edited_endorsement_content field
            # The Print Format should be designed to render doc.edited_endorsement_content
            pdf_data = frappe.get_print(
                'Project Proposal',
                doc.name,
                print_format=print_format, # Use the specified print format for styling/letterhead
                file_format='pdf',
                as_attachments=False # We want the binary data
            )

            # Create a new File DocType and attach it
            ...
            pdf_file = frappe.get_doc({
                ...
                "attached_to_field": "endorsement_attachment", # Optional: Link to a specific field if you have one
                "content": pdf_data,
                "is_private": 1 # Keep private for sensitive documents
            })
            pdf_file.insert(ignore_permissions=True) # Use insert for new docs
            frappe.db.commit() # Commit file creation
            ...
            # Continue without PDF if it fails, but log the error

        # 4. Forward to Dean, RnD role (no email) via ToDo
        dean_rnd_users = frappe.get_list('User', filters={'roles': ['Dean, RnD'], 'enabled': 1}, pluck='name')
        if dean_rnd_users:
            for user_id in dean_rnd_users:
                frappe.get_doc({
                    "doctype": "ToDo",
                    "description": f"Please review Endorsement for Project Proposal: {doc.name} - {doc.project_title}",
        ...
        frappe.throw(_("An unexpected error occurred during endorsement processing: {0}").format(str(e)))
