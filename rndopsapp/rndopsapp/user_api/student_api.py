...
def _fetch_academic_data(email):
	...
	try:
		...
		frappe.throw(_("Could not fetch student details from the Academic API: {0}").format(str(e)))
...
def add_student(email, projects=None):
	...
	try:
		...
		if student.get("d_name"):
			user.department_name = student.get("d_name")
...
# details before using the app. The PI-controlled part of `student_details`
...
def _student_profile_row(user=None):
	"""Return the student_details docname for a user, or None."""
	user = user or frappe.session.user
	username = _email_prefix(user)
	rows = frappe.get_all(
		"student_details", filters={"stu_id": username}, pluck="name", limit=1
	)
...
def get_my_student_profile():
	...
	data = {}
	if name:
		doc = frappe.get_doc("student_details", name)
...
def save_my_student_profile(data):
	...
	if name:
		...
		doc = frappe.new_doc("student_details")
		doc.stu_id = _email_prefix(user)
