...
class ResearchDepositSlipValidator:
    ...
    @staticmethod
    def validate_gst_details(dto: ResearchDepositSlipDTO) -> List[str]:
        """
        ...
        # GST details only required if gstType != NOGST
        if dto.gstType == "NOGST":
            return errors

        if not dto.gstDetails:
            # GST details should be present for GST types
            if dto.finalGstAmount > 0:
        ...
        if dto.gstType == "CGST_SGST":
            ...
            if not gst.cgstPercentage or gst.cgstPercentage <= 0:
                errors.append("cgstPercentage is required for CGST_SGST type")
            if not gst.sgstPercentage or gst.sgstPercentage <= 0:
                errors.append("sgstPercentage is required for CGST_SGST type")

        elif dto.gstType == "IGST":
            if not gst.igstAmount or gst.igstAmount <= 0:
                errors.append("igstAmount is required for IGST type")
            if not gst.igstPercentage or gst.igstPercentage <= 0:
                errors.append("igstPercentage is required for IGST type")

        # Validate totalGstAmount matches finalGstAmount
        total_gst = gst.totalGstAmount if gst.totalGstAmount is not None else 0.0
        final_gst = dto.finalGstAmount if dto.finalGstAmount is not None else 0.0
        if abs(total_gst - final_gst) > 0.01:
            errors.append(f"gstDetails.totalGstAmount ({gst.totalGstAmount}) should match finalGstAmount ({dto.finalGstAmount})")
