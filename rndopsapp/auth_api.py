...
import re
from rndopsapp.auth_middleware import jwt_auth_required
...
def _handle_failed_login(user_id):
    """Increment login attempts and lock account if >= 5"""
    try:
        current_attempts = frappe.db.get_value("Universal User__", user_id, "login_attempts_u_r") or 0
        new_attempts = current_attempts + 1
        
        if new_attempts >= 5:
            locked_until = add_to_date(now_datetime(), minutes=15)
            frappe.db.set_value("Universal User__", user_id, {
                "login_attempts_u_r": new_attempts,
                "locked_until_u_r": locked_until
            })
            frappe.logger().warning(f"User {user_id} locked out due to multiple failed attempts")
        else:
            frappe.db.set_value("Universal User__", user_id, "login_attempts_u_r", new_attempts)
        frappe.db.commit()
    except Exception as e:
        frappe.logger().error(f"Error handling failed login: {str(e)}")
...
def login(email, password, remember_me=False):
    ...
    try:
        ...
        locked_until = user.get("locked_until_u_r")
        if locked_until and locked_until > now_datetime():
            frappe.logger().warning(f"Login failed for {email}: Account locked (Until: {locked_until})")
            return {
                "status": "error",
        ...
        if getattr(user_doc, 'login_attempts_u_r', 0) > 0 or getattr(user_doc, 'locked_until_u_r', None):
            user_doc.login_attempts_u_r = 0
            user_doc.locked_until_u_r = None
        user_doc.save()
        frappe.db.commit()
...
def get_registration_progress():
    ...
    """
    try:
        universal_user_id = getattr(frappe.local, "request_user_id", None)
        if not universal_user_id:
            return {
                "completion_percentage": 0,
                "completed_sections": [],
                "pending_sections": ["basic", "contact", "bank", "education", "experience"]
            }
